...
router.post('/send-order-status', async (req, res) => {
    ...
    if (!email || !orderId || !status) {
        return res.status(400).json({ success: false, msg: 'Email, order ID, and status are required' });
    }
    ...
    const mailOptions = {
        ...
        text: `Your order with ID ${orderId} has been ${status}.`,
    };

    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            console.error('Error sending email:', error);
            return res.status(500).json({ success: false, msg: 'Error sending order status email' });
        }
        console.log('Email sent:', info.response);
        res.status(200).json({ success: true, msg: 'Order status email sent successfully' });
    });
});
